...
REAL(kind=PREC_DP), DIMENSION(:),ALLOCATABLE:: ref_para       ! Defined by DIFFEV
REAL(kind=PREC_DP), DIMENSION(0:MAXPAR)     :: kupl_para = 0.0 ! (0:MAXPAR)
REAL(kind=PREC_DP), DIMENSION(0:MAXPAR)     :: kupl_deriv= 0.0 ! (0:MAXPAR)
INTEGER                                     :: nrvalues = 0
REAL(kind=PREC_DP), DIMENSION(1:2, 0:15)    :: rvalues  = 0.0 ! (1:2)
