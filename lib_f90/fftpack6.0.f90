module fftpack_mod
!
!  FFT routines originally by Paul Swarztrauber
!
!  Author:
!
...
!
!  Version 6.0 by Reinhard Neder
!  Modifications:
!    - Made dummy and actual elements conform more strictly to each other
!      Might still need work on linear arrays that are mapped onto 
!      (multi)dimensional arrays.
!    - Replaced tabs by spaces
!    - Made function xercon a logical function and removed the local
!      definitions of "logical xercon"
!    - Placed fftpack into a module
!
!    > Significant changes are marked by "!RBN"
!
!    > To do in descending order of urgency
!
!      + Correct conditions on array lengths in comments
!      + Remove computed "go to"
!      + Remove "go to"
!      + Add intent to formal argument declarations
!      + Replace loop ends by "end do"
!      + Add label names to long do loops
!      + Create portable "kind" declarations
!      + Remove unneeded "return"
!      + Add subroutine name to "end" statement
!      + Turn declarations into "... :: ..." form
!
contains
!
...
!RBN subroutine c1fm1b ( n, inc, c, ch, wa, fnf, fac )
subroutine c1fm1b ( n, inc, lenc, lench, lenwa, iw1, c_in, ch, wa, fnf, fac )
...
!
  ...
  integer(kind=4), intent(in) :: lenc
  integer(kind=4), intent(in) :: lench
  integer(kind=4), intent(in) :: lenwa
  integer(kind=4), intent(in) :: iw1
!RBN  complex ( kind = 4 ) c(*)
!RBN  real ( kind = 4 ) ch(*)
!RBN  real ( kind = 4 ) fac(*)
  complex ( kind = 4 ) c_in(1:lenc)      !RBN the in/out complex array
!
  real ( kind = 4 ) c (1:2*lenc)         !RBN local real copy of c_in
  real ( kind = 4 ) ch(1:lench)          !RBN Added dimensions
  real ( kind = 4 ) fac(1:lenwa - iw1)   !RBN Added dimensions
  real ( kind = 4 ) fnf
  integer ( kind = 4 ) ido
  integer ( kind = 4 ) inc
  integer ( kind = 4 ) inc2
  integer ( kind = 4 ) ip
  integer ( kind = 4 ) iw
!RBN  integer ( kind = 4 ) k
  integer ( kind = 4 ) i                 !RBN Added loop counter
  integer ( kind = 4 ) k1
  integer ( kind = 4 ) l1
  integer ( kind = 4 ) l2
  integer ( kind = 4 ) lid
  ...
  integer ( kind = 4 ) nf
!RBN  real ( kind = 4 ) wa(*)
  real ( kind = 4 ) wa(1:lenwa)          !RBN Added dimensions
...
!RBN subroutine c1fm1f ( n, inc, c, ch, wa, fnf, fac )
subroutine c1fm1f ( n, inc, lenc, lench, lenwa, iw1, c_in, ch, wa, fnf, fac )
...
!RBN   call c1fm1b ( n, inc, c, work, wsave, wsave(iw1), wsave(iw1+1) )
  call c1fm1b ( n, inc, lenc, lenwrk, lensav,  iw1, &
                        c, work, wsave, wsave(iw1), wsave(iw1+1) )
...
!
  ...
  iw1 = n+n+1
!RBN  call c1fm1f (n,inc,c,work,wsave,wsave(iw1),wsave(iw1+1))
  call c1fm1f ( n, inc, lenc, lenwrk, lensav,  iw1, &
                        c, work, wsave, wsave(iw1), wsave(iw1+1) )
...
!RBN  call cmfm1b (lot,jump,n,inc,c,work,wsave,wsave(iw1),wsave(iw1+1))
  call cmfm1b (lot,jump,n,inc,lenc, lenwrk, lensav, iw1, &
               c,work,wsave,wsave(iw1),wsave(iw1+1))
...
!RBN  call cmfm1f (lot,jump,n,inc,c,work,wsave,wsave(iw1),wsave(iw1+1))
   call cmfm1f (lot,jump,n,inc, lenc, lenwrk, lensav, iw1, &
                c,work,wsave,wsave(iw1),wsave(iw1+1))
...
!
  ...
  integer ( kind = 4 ) im2
  integer ( kind = 4 ) k
!RBN  integer ( kind = 4 ) lid
  integer ( kind = 4 ) lot
  integer ( kind = 4 ) m1
  integer ( kind = 4 ) m1d
  integer ( kind = 4 ) m2
  integer ( kind = 4 ) m2s
!RBN  integer ( kind = 4 ) n
  integer ( kind = 4 ) na
  real ( kind = 4 ) sn
  real ( kind = 4 ) ti2
  real ( kind = 4 ) tr2
...
!RBN subroutine cmfm1b ( lot, jump, n, inc, c, ch, wa, fnf, fac )
subroutine cmfm1b ( lot, jump, n, inc, lenc, lench, lenwa, iw1, &
                    c_in, ch, wa, fnf, fac )
...
!RBN subroutine cmfm1f ( lot, jump, n, inc, c, ch, wa, fnf, fac )
subroutine cmfm1f ( lot, jump, n, inc, lenc, lench, lenwa, iw1, &
                    c_in, ch, wa, fnf, fac )
...
!
  ...
  integer ( kind = 4 ) n
  real ( kind = 4 ) work(lenwrk)
  real ( kind = 8 ) dwork(lenwrk)  ! RBN Added a double precision copy
  real ( kind = 4 ) wsave(lensav)
  real ( kind = 4 ) x(inc,*)
!RBN  logical xercon
  ...
  dwork = real(work, kind=8)
  iw1 = lot+lot+1
!RBN  call mcstb1(lot,jump,n,inc,x,wsave,work,work(iw1),ier1)
  call mcstb1(lot,jump,n,inc,x,wsave,dwork,work(iw1),ier1)
  work = real(dwork, kind=4)
...
!RBN  logical xercon
  ...
  dwork = real(work, kind=8)
!RBN  call mcstf1(lot,jump,n,inc,x,wsave,work,work(iw1),ier1)
  call mcstf1(lot,jump,n,inc,x,wsave,dwork,work(iw1),ier1)
  work = real(dwork, kind=4)
...
!
  ...
  real ( kind = 4 ) work(lenwrk)
  complex ( kind = 4 ), dimension(:), allocatable :: cwork   ! !RBN added complex copy
  real ( kind = 4 ) wsave(lensav)
...
!RBN       wsave(lwsav+1),mwsav,r,l*m, ier1)
      call cfftmb(ldh-1,1,m,ldh,cwork,ldh*m, &
           wsave(lwsav+1),mwsav,r,l*m, ier1)
      do loop = 1, ldh*m                      !RBN copy cwork bak into work
         work(2*loop)   = real(cwork(loop))
         work(2*loop+1) = imag(cwork(loop))
      enddo
      deallocate(cwork)                       !RBN   end new code
...
!RBN       wsave(lwsav+1),mwsav,r,l*m, ier1)
      call cfftmf(ldh-1,1,m,ldh,cwork,ldh*m, &
           wsave(lwsav+1),mwsav,r,l*m, ier1)
      do loop = 1, ldh*m                      !RBN copy cwork bak into work
         work(2*loop)   = real(cwork(loop))
         work(2*loop+1) = imag(cwork(loop))
...
!
  ...
  integer ( kind = 4 ) n
  real ( kind = 4 ) work(lenwrk)
  real ( kind = 8 ) dwork(lenwrk)     ! RBN Added double precision copy
  real ( kind = 4 ) wsave(lensav)
  real ( kind = 4 ) x(inc,*)
!RBN  logical xercon
  ...
  dwork = real(work, kind=8)
!RBN  call msntb1(lot,jump,n,inc,x,wsave,work,work(iw1),work(iw2),ier1)
  call msntb1(lot,jump,n,inc,x,wsave,dwork,work(iw1),work(iw2),ier1)
  work = real(dwork, kind=4)
...
!RBN  logical xercon
  ...
  iw2 = iw1 + lot * ( n + 1 )
  dwork = real(work, kind=8)
!RBN  call msntf1 ( lot, jump, n, inc, x, wsave, work, work(iw1), work(iw2), ier1 )
  call msntf1 ( lot, jump, n, inc, x, wsave, dwork, work(iw1), work(iw2), ier1 )
  work = real(dwork, kind=4)
...
logical function xercon ( inc, jump, n, lot )      !RBN added "logical" declaration
