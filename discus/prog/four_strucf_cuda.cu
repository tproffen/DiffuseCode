...
__global__ void four_strucf_cuda_kernel_2d(cuFloatComplex*, cuFloatComplex*,
					   int, int, int, int,
					   int, int, int, int);
...
extern "C"{
  void four_strucf_cuda_(cuFloatComplex *tcsf, cuFloatComplex *cex, float *xat, int *nxat, int *num, float *xm, float *win, float *vin, float *uin, int *cr_natoms)
  {
    ...
    for(int l=0; l< nxat[0]; l++){
      ...
      xincw = win[0] * xat[l] + win[1] * xat[l+cr_natoms[0]] + win[2] * xat[l+cr_natoms[0]*2];
      ...
      iincw = (int) rint( 64 * I2PI * (xincw - (int) xincw + 1.) );
      
      four_strucf_cuda_kernel<<<gridDim, blockDim>>>
	(d_cex, d_tcsf,
	 num[0],num[1],num[2],nnum,
	 iarg0,iincu,iincv,iincw);
    }
...
__global__ void four_strucf_cuda_kernel(cuFloatComplex* d_cex, cuFloatComplex* d_tcsf,
					int num1, int num2, int num3, int nnum,
					int iarg0, int iincu, int iincv, int iincw)
{
  int i, j, k, iadd, iarg;
  ...
  if(idx<nnum)
    {
      ...
      j = (idx / num3) % num2;
      k = idx % num3;
      iarg = iarg0 + i * iincu + j * iincv + k * iincw;
      iadd = iarg >> 6;
      iadd = iadd & MASK;
...
__global__ void four_strucf_cuda_kernel_2d(cuFloatComplex* d_cex, cuFloatComplex* d_tcsf,
					   int num1, int num2, int num3, int nnum,
					   int iarg0, int iincu, int iincv, int iincw)
{
  int i, j, iadd, iarg;
