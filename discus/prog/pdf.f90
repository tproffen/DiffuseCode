...
!RBN  IF (cr_at_lis (cr_iscat (1,ia) ) .ne.'VOID') then 
!RBN     bave = bave+form (cr_iscat (1,ia), cr_scat, pdf_lxray, hh,  pdf_power) *cr_occ(cr_iscat(1,ia))
...
!
      ...
      cr_n_real_atoms = 0 
      DO ia = 1, cr_natoms
         IF (cr_at_lis (cr_iscat (1,ia) ) .ne.'VOID') then 
            pdf_natoms(cr_iscat(1,ia)) = pdf_natoms(cr_iscat(1,ia)) + 1
         ENDIF 
      ENDDO 
!
      bave = 0.0
...
!        pdf_temp preserved accross its cycles!!!!!!!!!!!!!
         pdf_has_atom(:) = 0
         DO i=1, cr_natoms
            IF(pdf_allowed_i(cr_iscat(1,i)) .OR. pdf_allowed_j(cr_iscat(1,i)) ) THEN
               pdf_has_atom(cr_iscat(1,i)) = pdf_has_atom(cr_iscat(1,i)) + 1
            ENDIF
         ENDDO
         DO i=1, cr_nscat
            IF(pdf_has_atom(i)>0) pdf_nscat = i
         ENDDO
!      pdf_nscat = MAXVAL(cr_iscat(1,1:cr_natoms))
