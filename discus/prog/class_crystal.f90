...
!
   ...
   REAL(kind=PREC_DP), DIMENSION(:,:,:),ALLOCATABLE ::  cr_gen_add   ! Additional Generator matrices (4,4,0:192)
!
   INTEGER                                          ::  cr_sym_add_n         = 0
   INTEGER                                          ::  cr_sym_add_power(192) = 1
   REAL(kind=PREC_DP), DIMENSION(:,:,:),ALLOCATABLE ::  cr_sym_add   ! Additional Symmetry matrices (4,4,0:192)
!
...
!
   ...
   REAL(kind=PREC_DP)   , DIMENSION(:), ALLOCATABLE               ::  cr_mole_dens    ! (  0:MOLE_MAX_MOLE)
   REAL(kind=PREC_DP)   , DIMENSION(:), ALLOCATABLE               ::  cr_mole_biso    ! (  0:MOLE_MAX_TYPE)
   REAL(kind=PREC_DP)   , DIMENSION(:), ALLOCATABLE               ::  cr_mole_clin    ! (  0:MOLE_MAX_TYPE)
   REAL(kind=PREC_DP)   , DIMENSION(:), ALLOCATABLE               ::  cr_mole_cqua    ! (  0:MOLE_MAX_TYPE)
   REAL(kind=PREC_DP)   , DIMENSION(:), ALLOCATABLE               ::  cr_mole_fuzzy   ! (  0:MOLE_MAX_MOLE)
!
...
l_nanis   = max(nanis, l_nscat, 1)     ! Make array at least one entry long
...
!
   IF(this%latom) THEN                             ! Crystal was allocated 
      ...
      DEALLOCATE ( this%cr_mole_clin , STAT=istatus ) ! Always deallocate molecules
   ...
   ALLOCATE ( this%cr_mole_clin (0:n_type), STAT=istatus ) ! Allocate molecules
...
!******************************************************************************
   SUBROUTINE set_cryst_atom ( this, inum, itype, posit, iprop, isurface, magn_mom, at_value, iin_mole)
!
...
real(kind=PREC_DP)                 , intent(in) :: at_value
   INTEGER           , DIMENSION(1:2),INTENT(IN) :: iin_mole
!
   CALL this%atoms(inum)%set_atom ( itype, posit, iprop, isurface, magn_mom, at_value, iin_mole )
!
...
!
   CALL this%atoms(inum)%get_atom ( itype, posit, iprop, isurface, magn_mom, at_value, iin_mole )
...
!******************************************************************************
   SUBROUTINE get_cryst_atom ( this, inum, itype, posit, iprop, isurface, magn_mom, at_value, iin_mole)
!
...
!
   ...
   real(kind=PREC_DP)                , INTENT(OUT) :: at_value
   ...
   DO i=1, this%cr_num_mole
      DO j=1,this%cr_mole_len(i)
         IF ( this%cr_mole_cont(this%cr_mole_off(i)+j) == inum ) THEN
            ...
            r_m_clin = this%cr_mole_clin(this%cr_mole_type(i))
            r_m_cqua = this%cr_mole_cqua(this%cr_mole_type(i))
            RETURN
         ENDIF
      ENDDO
   ENDDO
...
!
   IF(this%cr_sav_mole .or. this%cr_sav_doma .or. this%cr_sav_obje) THEN
      ...
      FORALL ( i=0:mole_num_type)
         ...
         this%cr_mole_clin (i) = mole_clin (i)
         this%cr_mole_cqua (i) = mole_cqua (i)
      END FORALL
   ...
   ELSE
      ...
      this%cr_mole_clin     = 0.0
      this%cr_mole_cqua     = 0.0
...
!        if(.not.associated(this%atoms(ia))) allocate(this%atoms(ia))
      CALL this%atoms(ia)%set_atom ( itype, posit, iprop, isurface, magn_mom, at_value, iin_mole )
   ELSE
      IF(this%cr_sav_mole .OR. this%cr_sav_doma .OR. this%cr_sav_obje) THEN
         IF(cr_mole(inum)/=0) THEN
            ier_num = -157
...
!******************************************************************************
   SUBROUTINE set_crystal_from_local   ( this, strucfile, &
            ...
            rd_cr_pos, rd_cr_iscat, rd_cr_prop, rd_cr_surf, rd_cr_magn, rd_cr_valu, rd_cr_mole      &
            )
!
!
...
!
   ...
   REAL(kind=PREC_DP)  , DIMENSION(3,2)         , INTENT(IN) :: rd_cr_dim
   ...
   REAL(kind=PREC_DP)  , DIMENSION(3,3)         , INTENT(IN) :: rd_cr_gten
   REAL(kind=PREC_DP)  , DIMENSION(3,3)         , INTENT(IN) :: rd_cr_rten
   REAL(kind=PREC_DP)  , DIMENSION(3,3,3)       , INTENT(IN) :: rd_cr_eps
   REAL(kind=PREC_DP)  , DIMENSION(3,3,3)       , INTENT(IN) :: rd_cr_reps
   REAL(kind=PREC_DP)  , DIMENSION(3,3)         , INTENT(IN) :: rd_cr_gmat
   REAL(kind=PREC_DP)  , DIMENSION(3,3)         , INTENT(IN) :: rd_cr_fmat
   REAL(kind=PREC_DP)  , DIMENSION(4,4)         , INTENT(IN) :: rd_cr_tran_g
   REAL(kind=PREC_DP)  , DIMENSION(4,4)         , INTENT(IN) :: rd_cr_tran_gi
   REAL(kind=PREC_DP)  , DIMENSION(4,4)         , INTENT(IN) :: rd_cr_tran_f
   REAL(kind=PREC_DP)  , DIMENSION(4,4)         , INTENT(IN) :: rd_cr_tran_fi
   ...
   REAL(kind=PREC_DP),DIMENSION(11,0:rd_MAXSCAT), INTENT(IN) :: rd_cr_scat   ! (11,0:MAXSCAT)
   REAL(kind=PREC_DP)  , DIMENSION(0:rd_MAXSCAT), INTENT(IN) :: rd_cr_delfr  ! (  0:MAXSCAT)
   REAL(kind=PREC_DP)  , DIMENSION(0:rd_MAXSCAT), INTENT(IN) :: rd_cr_delfi  ! (  0:MAXSCAT)
   ...
   REAL(kind=PREC_DP)  , DIMENSION(    rd_NMAX) , INTENT(IN) :: rd_cr_valu
...
!
   ...
   IF ( this%cr_sav_mole .or. this%cr_sav_obje .or. this%cr_sav_doma ) THEN
      FORALL ( ia=0:mole_num_type)
         ...
         mole_cqua (ia) = this%cr_mole_cqua (ia)
      END FORALL
