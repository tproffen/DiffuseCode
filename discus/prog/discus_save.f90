...
!
      if(lpresent(O_FORMAT)) then             ! User provided format:
         if(str_comp(opara(O_FORMAT), 'keyword', 1, lopara(O_FORMAT), 7)) then
            sav_keyword = .true.
            is_style = 0
         elseif(str_comp(opara(O_FORMAT), 'nexus', 3, lopara(O_FORMAT), 5)) then
            sav_keyword = .true.
            is_style = 1
         elseif(str_comp(opara(O_FORMAT), 'nokeyword', 3, lopara(O_FORMAT), 9)) then
            sav_keyword = .false.
            is_style = 2
         endif
      else
      ...
      IF (sav_keyword) THEN 
         IF(sav_w_scat .EQV. sav_w_adp .AND. &
            ...
            elseif(is_style==1) then
               CALL save_keyword_unified(sav_file) 
...
!********************************************************************** 
!
subroutine save_keyword_unified(outfile) 
!-
! Interface to the generic NeXuS nx_write_structure
!+
!
use celltoindex_mod
use chem_mod
...
logical, dimension(8) :: optional_intended
!
character(len=4)      :: element   ! Proper element name
integer, dimension(3) :: icell     ! Atom is in this unit cell
integer               :: isite     ! Atom is on this site
...
optional_intended(o_crystal_meta) = .true.
!
unit_cell_lengths  = cr_a0
unit_cell_angles   = cr_win
!
symmetry_H_M       = cr_spcgr
symmetry_origin    = 1   ! WORK
symmetry_abc       = cr_set
...
if(nint(res_para(1))>0) then
   n_void = 1
else
   n_void = 0
endif
number_of_types    = cr_nscat
allocate(types_names    (number_of_types+n_void))
allocate(types_ordinal  (number_of_types+n_void))
allocate(types_charge   (number_of_types+n_void))
...
do i=1, number_of_types
   ...
   if(j<0) then
      ier_typ = ER_APPL
      ier_num = -204
      ier_msg(1) = 'Unknown element : ' // element
      return
   endif
   types_ordinal(i)  = get_ordi(element)
   j = len_trim(cr_at_lis(i))
   if(cr_at_lis(i)(j:j)=='+' .or. &
      cr_at_lis(i)(j:j)=='-' ) then
      read(cr_at_lis(i)(j-1:j-1),'(i1)') types_charge(i)
   endif
   types_isotope(i) = 0
...
if(n_void==1) then
   ...
   types_names    (number_of_types) = 'VOID'
   types_ordinal  (number_of_types) = 0
   types_charge   (number_of_types) = 0
   types_isotope  (number_of_types) = 0
   types_occupancy(number_of_types) = 1.00_PREC_DP
endif
optional_intended(o_types_occupancy) = sav_w_occ
!
number_of_atoms = cr_natoms
...
do i=1, number_of_atoms
   ...
   atom_pos      (:,i) = cr_pos(:,i)
   atom_unit_cell(:,i) = icell
   atom_site     (  i) = isite
   atom_property (  i) = cr_prop(i)
enddo
optional_intended(o_atom_property) = sav_w_prop
!
optional_intended(o_crystal_flags) = .true.
crystal_flags = .false.
...
if(chem_period(3)) crystal_flags(1,5) = .true.
crystal_flags(2,3:5) = .true.          ! Flags for periodic boundary conditions are set
!
crystal_flags(:,6) = cr_flags(:,6)    ! statement on homogeneity   !NEXUS WORK
!
...
if(cr_nanis>0) then                    ! Anisotropic ADPs  (also for Uiso
   ...
   anisotropic_adp%anisotropic_adp    = 0.0_PREC_DP   ! Default
   anisotropic_adp%anisotropic_adp(1:6, 1:anis_n) = cr_anis_full(1:6, 1:anis_n)
   do i=1, anis_n
      if(all(cr_anis_full(2:6,i)<TOL)) then      ! Isotropic only
         anisotropic_adp%anisotropic_is_iso = 0
      else
         anisotropic_adp%anisotropic_is_iso = 1
      endif
      anisotropic_adp%anisotropic_adp(7  ,i) = (cr_prin(4,1,i)+cr_prin(4,2,i)+cr_prin(4,3,i))/3.0_PREC_DP
   enddo
   do i=1, number_of_atoms
      anisotropic_adp%atom_index(i)  = cr_iscat(3,i)
   enddo
   optional_intended(o_anisotropic_adp) = .true.
...
if(mole_num_mole > 0) then
   ...
   j = maxval(mole_len(1:mole_num_mole))
...
write(*,*) ' MOLECIULE ', i, mole_type(i), ' >>> ', mole_num_mole, mole_num_type, mole_num_atom
      ...
      molecules%mole_real(1,mole_type(i))  = mole_biso(mole_type(i))/(8.*PI**2)
      molecules%mole_real(2,mole_type(i))  = mole_clin(mole_type(i))
      molecules%mole_real(3,mole_type(i))  = mole_cqua(mole_type(i))
      do j=1, mole_len(i)
         molecules%atom_index(j,i) = mole_cont(mole_off(i)+j)
      enddo
   enddo
   optional_intended(o_molecules) = sav_w_mole
else
   optional_intended(o_molecules) = .false.
endif
!
!  Average structure
!
if(.not.crystal_flags(1,2) .and.    &  !   Not an asymmetric unit
        crystal_flags(1,6)      ) then   ! Homogeneous
   call chem_aver(loutput, lsite)
   average_struc%aver_n_atoms = cr_ncatoms
   ...
   optional_intended(o_average_struc) = .false.
endif
!
!  Optional magnetic scattering vectors
!
...
!
end subroutine save_keyword_unified
!
!********************************************************************** 
!
