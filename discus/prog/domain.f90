...
!                                                                       
      CALL stru_readheader (imd, MMAX, MK_MAX_SCAT, lcell, mk_name,     &
...
!     Read the input file header                                        
!                                                                       
      CALL stru_readheader (ist, MMAX, MK_MAX_SCAT, lcell, mk_name,     &
      mk_spcgr, mk_at_lis, mk_nscat, mk_dw, mk_a0, mk_win, sav_ncell,   &
      sav_r_ncell, sav_ncatoms, mk_spcgr_ianz, mk_spcgr_para)           
!                                                                       
!DBG_RBN                                                                
!DBG      do i=1,4                                                      
!DBG      write(*,'(a,4f10.5)') ' CRYSTAL MATRIX ',                     
!DBG  &           (mc_matrix(i,j),j=1,4)                                
!DBG      ENDDO                                                         
!DBG      do i=1,4                                                      
!DBG      write(*,'(a,4f10.5)') ' DIMEN   MATRIX ',                     
!DBG  &           (mc_dimen (i,j),j=1,4)                                
!DBG      ENDDO                                                         
!DBG      do i=1,4                                                      
!DBG      write(*,'(a,4f10.5)') ' IDIMEN  MATRIX ',                     
!DBG  &           (mc_idimen(i,j),j=1,4)                                
!DBG      ENDDO                                                         
!DBGREMOE_OLD                                                           
!DBG      call micro_remove_old(mc_idimen)                              
      CALL micro_read_atom (ist, mc_idimen, mc_matrix) 
      CLOSE (ist) 
!                                                                       
...
!                                                                       
      DO i = 1, cr_natoms 
!DBG_RBN                                                                
!DBG      if(i.eq.1689.or.i.eq.cr_natoms) then                          
!DBG        write(*,*) ' Atom ',(cr_pos(j,i),j=1,3)                     
!DBG      endif                                                         
      DO j = 1, 3 
      u4 (j) = cr_pos (j, i) 
      ENDDO 
...
!*****7*****************************************************************
      SUBROUTINE micro_read_atom (ist, mc_idimen, mc_matrix) 
!                                                                       
      USE config_mod 
      USE allocate_appl_mod 
      USE crystal_mod 
...
!                                                                       
         ...
         ELSE 
            ...
            ENDDO 
!DBG_RBN                                                                
!DBG      if(i_count.eq.1 .or.((u(1).eq.0 .or. u(1).eq.1) .and.         
!DBG  &           u(2).eq.0 .and. u(3).eq.0)) then                      
!DBG        write(*,*) ' wert',werte                                    
!DBG        write(*,*) ' matrix(1)   ',(mc_matrix(1,j),j=1,4)           
!DBG        write(*,*) ' matrix(2)   ',(mc_matrix(2,j),j=1,4)           
!DBG        write(*,*) ' matrix(3)   ',(mc_matrix(3,j),j=1,4)           
!DBG        write(*,*) ' u   ',u                                        
!DBG        write(*,*) ' vv  ',vv                                       
!DBG        write(*,*) ' w   ',w                                        
!DBG        write(*,*) ' v   ',v                                        
...
!DBG_RBN                                                                
!DBG      if(i_count.eq.1) then                                         
!DBG        write(*,*) ' d,linside ',d,linside                          
!DBG      endif                                                         
            ELSEIF (mc_type  .eq.MD_DOMAIN_CUBE) then 
...
!                                                                       
               ...
               i = cr_natoms 
!DBGmc_orig
!WRITE(*,*) 'Getting atom at pos v  ', i      , (v      (j       ),j=1,3)
               DO j = 1, 3 
               cr_pos (j, i) = v (j)
