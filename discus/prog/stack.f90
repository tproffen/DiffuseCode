...
!
                  ...
                  loop_search: DO i = 1, st_nchem 
                     IF ('internal_stack_layer.'//cpara(1)(1:len_trim(cpara(1))) == &
                         st_layer_c(i)(1:len_trim(st_layer_c(i))) ) then 
                        st_chem (st_ntypes) = i 
                        lsuccess = .true.
                        exit loop_search
                     ENDIF 
                  ENDDO  loop_search
                  if(.not.lsuccess) then      ! New layer chemistry
                     st_nchem = st_nchem + 1 
                     st_chem (st_ntypes) = st_nchem 
                     st_layer_c(st_nchem) = 'internal_stack_layer.'//cpara(1)(1:lpara(1))
!QQQ              endif
!
                     i = st_ntypes
...
!call chem_elem(.true.)
                     ...
                     st_internal(i) = .true.
!write(*,'(a,i3, a)') 'LAYER ', i, st_layer(i)(1:len_trim(st_layer(i)))
!write(*,'(a,i3, a)') 'Layer ', i, st_layer_c(st_nchem)(1:len_trim(st_layer_c(st_nchem)))
                  else                        ! Old layer chemistry
                     st_layer(st_ntypes) = st_layer_c(st_chem (st_ntypes))
                     st_llayer = len_trim(st_layer(st_ntypes))
                     st_internal(st_ntypes) = .true.
!write(*,'(a,i3, a)') 'LAYER ', st_ntypes, st_layer(st_ntypes)(1:len_trim(st_layer(st_ntypes)))
!write(*,'(a,i3, a)') 'Layer ', st_ntypes, st_layer_c(st_nchem)(1:len_trim(st_layer_c(st_nchem)))
                  endif
!
!QQQ              i = st_nchem
!QQQ              strucfile = st_layer_c(i)
!QQQ              call read_to_internal(strucfile, 'internal_stack_layer.')
!QQQ              st_layer_c(i) = 'internal_stack_layer.'//st_layer_c(i)(1:len_trim(st_layer_c(i)))
!
               ELSE 
                  ier_num = - 53 
                  ier_typ = ER_APPL 
...
!
st_nanis = 0
st_is_stack = .true.             ! The list is generated by stacking faults
cr_is_stack    = .true.
CALL save_internal(tempfile)        ! temporarily save current structure
cr_is_stack = .false.               ! The actual crystal is not yet created by stacking faults
!
natoms = st_nlayer
nscats = st_ntypes
need_alloc = .false.
...
!read(*,*) i
   loop_st_ntypes: do i=1, st_ntypes
      ...
      CALL stru_readheader_internal(st_layer(i), DIM_NSCAT, DIM_NCATOMS, DIM_NANIS, st_name, &
               ...
               st_magnetic, st_is_anis, st_is_asym, st_is_homo, st_is_stack,&
...
!                                                                       
   cr_is_stack = .true.       ! The crystal has been build with stacking faults
   CALL update_cr_dim 
...
!write(*,*) ' ESTIMATE UNIT CELLS ', n_cells
!write(*,*) ' ESTIMATE NCATOMS    ', cr_natoms, cr_ncatoms
!
